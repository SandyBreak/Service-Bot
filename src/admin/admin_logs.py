    ...
    if not(id_topic_chat):
        try:
            ...
            if 'not enough rights' in str(e):
                    ...
                    await bot.send_message(chat_id=PRIMARY_GROUP_ID, text=f'{Emojis.ALLERT} Обнаружен новый пользователь! {Emojis.ALLERT}\n\nЯ не могу создать новый диалог с ним из-за того у моих прав недостаточно!\n\nНазначьте меня администратором!')
                    return
        ...
        try:
            await bot.copy_message(chat_id=PRIMARY_GROUP_ID, from_chat_id=message_log.chat.id, message_id=message_log.message_id, message_thread_id=id_topic_chat, protect_content=None)
        except TelegramBadRequest as e:
                '''
                ...
                if 'message thread not found' in str(e):
                    ...
                    try:
                        ...
                        if 'not enough rights' in str(e):
                                logging.error(f'Not enough rights!')
                                await bot.send_message(chat_id=PRIMARY_GROUP_ID, text='Назначьте меня администратором!')
                                return
                    except Exception as e:
                        logging.error(f'Error create forum topic: {e}')
